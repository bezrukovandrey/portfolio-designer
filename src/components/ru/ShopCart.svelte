        ...
        <div class="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
          <div class="flex flex-col items-stretch w-6/12 max-md:w-full max-md:ml-0 py-20">
            {#if cartItems.length > 0}
                    ...
                    <div class="gap-5 flex max-md:flex-col max-md:items-stretch max-md:gap-0">
                      ...
                      <div class="flex flex-col items-stretch w-1/2 ml-6 max-md:w-full max-md:ml-0">
                        <div class="items-stretch flex flex-col max-md:mt-10">
                          <div class="text-mainBrown font-ubuntu text-l">{item.title_ru}</div>
                          <div class="text-mainBrown font-ubuntu text-l whitespace-nowrap mt-10">{item.price_ru} ₽</div>
                        </div>
                      </div>
                    </div>
                  </div>
              ...
              {/each}
            {:else}
              <p class="text-mainBrown text-xxl font-ubuntu font-medium text-center">Ваша корзина пуста.</p>
            {/if}
          </div>
        {#if cartItems.length > 0}
      <div class="flex flex-col items-stretch w-1/2 ml-5 max-md:w-full max-md:ml-0">
        <div class="justify-between items-stretch flex grow flex-col px-1.5 py-8 max-md:max-w-full max-md:mt-8">
          <div class="justify-between flex gap-5 px-6 items-start max-md:max-w-full max-md:flex-wrap max-md:px-5">
            <div class="text-mainBrown text-xxl font-ubuntu font-medium">К оплате:</div>
            <div class="text-mainBrown text-xxl font-ubuntu font-medium">{priceTotal()} ₽</div>
          </div>
          <button><a href="/shop" class="text-mainBeige w-auto max-w-full py-4 px-6 mt-8 text-l font-bitter font-medium bg-lightBrown  hover:bg-mainBrown rounded-lg text-center">ПРОДОЛЖИТЬ ПОКУПКИ</a></button>
          <button><a href="#" class="text-mainBeige w-auto max-w-full py-4 px-6 mt-8 text-l font-bitter font-medium bg-mainBrown  hover:bg-lightBrown active:bg-darkBrown rounded-lg text-center">ОФОРМИТЬ ЗАКАЗ</a></button>
        </div>
      </div>
      {/if}
    </div>
