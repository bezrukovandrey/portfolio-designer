<script>
    ...
    onMount(async () => {
      try {
        ...
        portfolio = data.portfolio.filter(item => item.id >= 7 && item.id <= 9); 
      } catch (error) {
  ...
  <div class="items-stretch self-center flex flex-wrap w-full max-w-[960px] gap-4 mt-8 px-5 max-md:max-w-full max-md:justify-center max-md:flex-col">
    {#each portfolio as item}
      <div
        class="relative rounded-3xl border-mainBrown border-2 overflow-hidden min-h-[360px] grow"
      ...
      >
        ...
        {/if}
        <h1 class="text-mainBeige font-ubuntu text-2xl absolute bottom-7 left-9 text-shadow">{item.title_en}</h1>
      </div>
    {/each}
  </div>
